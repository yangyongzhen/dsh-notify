...
function textOf(content: readonly TextBlockLike[]): string {
	const parts: string[] = [];
	for (const block of content) {
		if (block.type === 'text' && block.text !== undefined) parts.push(block.text);
		else if (block.type === 'tool-result' && block.content !== undefined) parts.push(textOf(block.content));
	}
	return parts.join('\n\n');
}
...
export function summarize(events: readonly SessionEvent[]): NotifySummary {
	...
	for (const event of events) {
		...
		switch (event.type) {
			...
			case 'turn/end': reason = event.data.reason.kind; break;
...
export function buildMessage(summary: NotifySummary, maxChars: number, titlePrefix: string): { title: string; content: string } {
	...
	const lines: string[] = [];
	if (summary.model !== undefined) lines.push(`模型：${summary.model}`);
	lines.push(`耗时：${fmtDuration(summary.durationMs)}`);
	if (summary.toolCalls > 0) lines.push(`工具调用：${summary.toolCalls} 次`);
 ...
 */
export async function sendNotification(channel: NotifyChannel, message: { title: string; content: string }): Promise<void> {
	...
	try {
		...
		switch (channel.type) {
			...
			case 'dingtalk': {
				if (channel.secret !== undefined) url = `${channel.url}${channel.url.includes('?') ? '&' : '?'}${dingtalkSignature(channel.secret).slice(1)}`;
